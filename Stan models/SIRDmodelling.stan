...
# Italy went into lockdown on the 39th day since first cases
data <- data[1:39,]
...
# data for Stan
modeldatasird <- list(n_days = n_days, y0 = y0, t0 = t0, ts = t, N = N, infected = infected,
                     recovered=recovered, deaths=deaths)

model <- stan_model("finalsird.stan", verbose=TRUE)

# start sampling process 
fitsird <- sampling(model, data = modeldatasird, iter = 5000, warmup = 2000, chains = 4,
                   verbose=TRUE)

# print results from the Stan model
pars=c('beta','delta','R0')
print(fitsird, pars = pars)

# Check whether there were any divergences
rstan::check_divergences(fitsird)

# Check whether the chains are in agreement with each other
stan_dens(fitsird, pars = c("beta","delta"), separate_chains = TRUE)

# plot mean, median, CI and observed values for infected and removed
predictedinfected <- cbind(as.data.frame(summary(
  fitsird, pars = "pred_infected", probs = c(0.05, 0.5, 0.95))$summary), t, infected)
...
predictedrecovered <- cbind(as.data.frame(summary(
  fitsird, pars = "pred_recovered", probs = c(0.05, 0.5, 0.95))$summary), t, recovered)
...
predicteddeaths <- cbind(as.data.frame(summary(
  fitsird, pars = "pred_deaths", probs = c(0.05, 0.5, 0.95))$summary), t, deaths)
