...
functions {
  real[] sir(real t, real[] y, real[] theta, 
      ...
      real dI_dt =  beta * I * S / N - 0.2 * I - delta*I;
      real dR_dt =  0.2 * I;
      real dD_dt = delta*I;
...
model {
  ...
  recovered ~ neg_binomial_2(col(to_matrix(y), 3), phiR);
  deaths ~ neg_binomial_2(col(to_matrix(y), 4), phiD);
}

generated quantities {
  real R0 = beta / 0.2;
  real pred_infected[n_days];
  real pred_recovered[n_days];
  real pred_deaths[n_days];
  pred_infected = neg_binomial_2_rng(col(to_matrix(y), 2) + 1e-6, phiI);
  pred_recovered = neg_binomial_2_rng(col(to_matrix(y), 3) + 1e-6, phiR);
  pred_deaths = neg_binomial_2_rng(col(to_matrix(y), 4) + 1e-6, phiD);
}
